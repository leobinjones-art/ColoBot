 ...
 * 创建习惯
 */
export function createHabit(userId: string, name: string, frequency: HabitFrequency = 'daily', db?: Database.Database): Habit {
...
export function getHabit(id: string, userId: string, db?: Database.Database): Habit | null {
  const database = db || getDb();
  const row = database.prepare(`SELECT * FROM assistant_habits WHERE id = ? AND user_id = ?`).get(id, userId) as any;
...
export function listHabits(userId: string, db?: Database.Database): Habit[] {
  const database = db || getDb();
  const rows = database.prepare(`SELECT * FROM assistant_habits WHERE user_id = ? ORDER BY created_at ASC`).all(userId) as any[];
