 ...
 * 记录收支
 */
export function logFinance(userId: string, type: FinanceType, amount: number, category?: string, note?: string, db?: Database.Database): FinanceEntry {
 ...
 * 获取收支记录
 */
export function getFinanceEntries(userId: string, type?: FinanceType, limit = 30, db?: Database.Database): FinanceEntry[] {
  const database = db || getDb();
 ...
 * 获取财务统计
 */
export function getFinanceStats(userId: string, startDate?: string, endDate?: string, db?: Database.Database): FinanceStats {
  const database = db || getDb();
