 ...
 * 记录健康数据
 */
export function logHealth(userId: string, type: HealthEntry['type'], value: number, unit: string, note?: string, db?: Database.Database): HealthEntry {
 ...
 * 记录运动
 */
export function logExercise(userId: string, minutes: number, note?: string, db?: Database.Database): HealthEntry {
 ...
 * 记录睡眠
 */
export function logSleep(userId: string, hours: number, note?: string, db?: Database.Database): HealthEntry {
 ...
 * 记录体重
 */
export function logWeight(userId: string, kg: number, note?: string, db?: Database.Database): HealthEntry {
 ...
 * 获取健康记录
 */
export function getHealthEntries(userId: string, type?: HealthEntry['type'], limit = 30, db?: Database.Database): HealthEntry[] {
  const database = db || getDb();
 ...
 * 获取健康统计
 */
export function getHealthStats(userId: string, days = 7, db?: Database.Database): {
