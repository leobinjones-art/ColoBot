 ...
 * 创建密码条目
 */
export function createPasswordEntry(userId: string, name: string, password: string, options: { username?: string; url?: string; note?: string } = {}, db?: Database.Database): PasswordEntry {
 ...
 * 获取密码条目（不含密码）
 */
export function getPasswordEntry(id: string, userId: string, db?: Database.Database): PasswordEntry | null {
  const database = db || getDb();
  const row = database.prepare(`SELECT id, user_id, name, username, url, note, created_at, updated_at FROM assistant_passwords WHERE id = ? AND user_id = ?`).get(id, userId) as any;
...
export function getPassword(id: string, userId: string, db?: Database.Database): string | null {
  const database = db || getDb();
  const row = database.prepare(`SELECT encrypted_password FROM assistant_passwords WHERE id = ? AND user_id = ?`).get(id, userId) as any;
  if (!row) return null;
...
export function listPasswordEntries(userId: string, db?: Database.Database): PasswordEntry[] {
  const database = db || getDb();
  const rows = database.prepare(`SELECT id, user_id, name, username, url, note, created_at, updated_at FROM assistant_passwords WHERE user_id = ? ORDER BY name ASC`).all(userId) as any[];
 ...
 * 更新密码条目
 */
export function updatePasswordEntry(id: string, userId: string, updates: { name?: string; username?: string; password?: string; url?: string; note?: string }, db?: Database.Database): PasswordEntry | null {
  const database = db || getDb();
  ...
  if (updates.password) { fields.push('encrypted_password = ?'); values.push(encrypt(updates.password)); }
