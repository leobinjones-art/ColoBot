 ...
 * 开始时间追踪
 */
export function startTimeLog(userId: string, activity: string, category?: string, note?: string, db?: Database.Database): TimeLog {
...
export function getTimeLog(id: string, userId: string, db?: Database.Database): TimeLog | null {
  const database = db || getDb();
  const row = database.prepare(`SELECT * FROM assistant_time_logs WHERE id = ? AND user_id = ?`).get(id, userId) as any;
...
export function getActiveTimeLogs(userId: string, db?: Database.Database): TimeLog[] {
  const database = db || getDb();
  const rows = database.prepare(`SELECT * FROM assistant_time_logs WHERE user_id = ? AND ended_at IS NULL ORDER BY started_at DESC`).all(userId) as any[];
 ...
 * 获取时间记录列表
 */
export function getTimeLogs(userId: string, startDate?: string, endDate?: string, limit = 50, db?: Database.Database): TimeLog[] {
  const database = db || getDb();
 ...
 * 获取时间统计
 */
export function getTimeStats(userId: string, startDate?: string, endDate?: string, db?: Database.Database): {
  totalMinutes: number;
