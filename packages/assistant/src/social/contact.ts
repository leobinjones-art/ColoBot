 ...
 * 创建联系人
 */
export function createContact(userId: string, name: string, options: Partial<Contact> = {}, db?: Database.Database): Contact {
...
export function getContact(id: string, userId: string, db?: Database.Database): Contact | null {
  const database = db || getDb();
  const row = database.prepare(`SELECT * FROM assistant_contacts WHERE id = ? AND user_id = ?`).get(id, userId) as any;
 ...
 * 更新联系人
 */
export function updateContact(id: string, userId: string, updates: Partial<Contact>, db?: Database.Database): Contact | null {
  const database = db || getDb();
...
export function deleteContact(id: string, userId: string, db?: Database.Database): boolean {
  const database = db || getDb();
  return database.prepare(`DELETE FROM assistant_contacts WHERE id = ? AND user_id = ?`).run(id, userId).changes > 0;
}

/**
 * 记录互动
 */
export function recordInteraction(id: string, userId: string, db?: Database.Database): Contact | null {
