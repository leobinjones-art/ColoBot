 ...
 * 创建项目
 */
export function createProject(userId: string, name: string, description?: string, db?: Database.Database): Project {
...
export function getProject(id: string, userId: string, db?: Database.Database): Project | null {
  const database = db || getDb();
  const row = database.prepare(`SELECT * FROM assistant_projects WHERE id = ? AND user_id = ?`).get(id, userId) as any;
 ...
 * 更新项目
 */
export function updateProject(id: string, userId: string, updates: Partial<Pick<Project, 'name' | 'description' | 'status' | 'progress'>>, db?: Database.Database): Project | null {
  const database = db || getDb();
 ...
 * 列出项目
 */
export function listProjects(userId: string, status?: ProjectStatus, db?: Database.Database): Project[] {
  const database = db || getDb();
...
export function deleteProject(id: string, userId: string, db?: Database.Database): boolean {
  const database = db || getDb();
  return database.prepare(`DELETE FROM assistant_projects WHERE id = ? AND user_id = ?`).run(id, userId).changes > 0;
}
