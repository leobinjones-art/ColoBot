...
env.cacheDir = process.env.TRANSFORMERS_CACHE || path.join(process.env.HOME || '/tmp', '.cache', 'transformers');
 ...
 * OCR 文字识别
 */
async function performOCR(imageBuffer: Buffer, language?: string): Promise<{ text: string; confidence: number }> {
  ...
  try {
    ...
    console.log(`[Vision] OCR completed in ${Date.now() - startTime}ms, confidence: ${confidence.toFixed(2)}`);
 ...
 * 图像分类
 */
async function performClassification(imageBuffer: Buffer): Promise<{ labels: string[]; confidence: number }> {
  try {
...
export async function analyzeImageLocal(
  ...
  try {
    ...
    if (results.labels?.length || results.text) {
      ...
      if (results.text) {
        parts.push(`识别文字: ${results.text.slice(0, 200)}${results.text.length > 200 ? '...' : ''}`);
      }
