...
// 默认日志目录
const LOG_DIR = path.join(process.env.HOME || '', '.colomind', 'logs')
...
export function createCliLogger(config: Partial<LoggerConfig> = {}): Logger {
  return new Logger({
    ...
    level: (process.env.COLOMIND_LOG_LEVEL as LogLevel) || 'info',
    console: process.env.COLOMIND_LOG_CONSOLE === 'true',
    ...config,
  })
}
