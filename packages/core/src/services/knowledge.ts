...
export async function searchKnowledge(
  ...
  const rows: Row[] = category
    ? await query(
        `SELECT * FROM knowledge_base WHERE category = $1 AND (name ILIKE $2 OR content ILIKE $2) ORDER BY name`,
        [category, pattern],
      )
    : await query(
        `SELECT * FROM knowledge_base WHERE name ILIKE $1 OR content ILIKE $1 ORDER BY name`,
        [pattern],
      )
  return rows.map((row) => ({
    id: row.id,
