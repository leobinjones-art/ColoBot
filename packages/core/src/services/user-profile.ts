...
export async function getUserProfile(agentId: string): Promise<UserProfile | null> {
  const row = await queryOne<{
    id: string; agent_id: string; name: string | null; role: string | null;
    organization: string | null; bio: string | null; expertise_level: string | null;
    research_fields: string | string[] | null; skills: string | string[] | null;
    languages: string | string[] | null; communication_style: string | null;
    response_length: string | null; preferred_language: string | null;
    goals: string | string[] | null; current_projects: string | string[] | null;
    created_at: string; updated_at: string;
  }>('SELECT * FROM user_profiles WHERE agent_id = $1', [agentId]);
...
export async function upsertUserProfile(agentId: string, update: ProfileUpdate): Promise<UserProfile> {
  ...
  if (existing) {
    ...
    if (updates.length > 0) {
      ...
      values.push(agentId);
      await query(`UPDATE user_profiles SET ${updates.join(', ')} WHERE agent_id = $${paramIndex}`, values);
    }
    ...
    await query(
      ...
      [
        id, agentId, update.name || null, update.role || null, update.organization || null,
        update.bio || null, update.expertise_level || null,
        JSON.stringify(update.research_fields || []), JSON.stringify(update.skills || []),
        JSON.stringify(update.languages || []), update.communication_style || null,
        update.response_length || null, update.preferred_language || null,
        JSON.stringify(update.goals || []), JSON.stringify(update.current_projects || []),
      ]
    );

    const profile = await getUserProfile(agentId);
    if (profile) await syncProfileToMemory(agentId, profile);
    return profile!;
  }
}
...
export function buildProfilePrompt(profile: UserProfile | null): string {
  ...
  if (profile.research_fields?.length) parts.push(`研究领域：${profile.research_fields.join('、')}。`);
  if (profile.skills?.length) parts.push(`擅长技能：${profile.skills.join('、')}。`);
  return parts.join('\n');
}
...
function getRoleLabel(role: UserRole): string {
  const labels: Record<UserRole, string> = {
    student: '学生', researcher: '研究人员', developer: '开发者', writer: '写作者', other: '其他',
  };
...
function getExpertiseLabel(level: ExpertiseLevel): string {
  const labels: Record<ExpertiseLevel, string> = {
    beginner: '初学者', intermediate: '中级', expert: '专家',
  };
