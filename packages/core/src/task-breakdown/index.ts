...
export const DEFAULT_TOOLS: ToolDefinition[] = [
  {
    name: 'web_search',
    description: '网络搜索',
    capabilities: ['搜索', '查询', '查找', '天气', '新闻', '实时'],
  },
  {
    name: 'read_file',
    description: '读取文件',
    capabilities: ['读取', '文件', '表格', 'CSV', 'Excel', 'JSON'],
  },
  {
    name: 'write_file',
    description: '写入文件',
    capabilities: ['保存', '写入', '导出', '生成文件'],
  },
  {
    name: 'python',
    description: 'Python执行',
    capabilities: ['计算', '分析', '统计', '数据处理', '可视化'],
  },
...
export async function analyzeRequest(
  ...
  const prompt = ANALYSIS_PROMPT.replace('{tools}', toolsDesc).replace(
    '{request}',
 ...
 * 构建子任务的提示词（包含依赖的输出）
 */
function buildSubTaskPrompt(subTask: SubTask, ctx: ExecutionContext): string {
...
export async function executeDynamicTask(
  ...
  while (executed.size + failed.size < result.analysis.subTasks.length) {
    ...
    const ready = result.analysis.subTasks.filter(
      (st) =>
    ...
    if (ready.length === 0) {
      // 检查是否有依赖失败导致无法继续
      const blocked = result.analysis.subTasks.filter(
        (st) =>
          !executed.has(st.name) &&
          !failed.has(st.name) &&
...
function summarizeResults(result: TaskResult): string {
  ...
  for (const subTask of result.analysis.subTasks) {
    ...
    if (execResult?.output) {
      lines.push(
        `   ${execResult.output.slice(0, 200)}${execResult.output.length > 200 ? '...' : ''}`,
      )
    }
    lines.push('')
  }
