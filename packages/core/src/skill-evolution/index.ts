...
export async function detectPatterns(
  ...
  const existing = await query<SkillPattern>(
    `SELECT * FROM skill_patterns WHERE agent_id = $1 AND tool_sequence = $2`,
    [agentId, toolSeqStr],
  )

  if (existing.length > 0) {
    const confidence = Math.min(1, existing[0].confidence + 0.1)
    await query('UPDATE skill_patterns SET confidence = $1 WHERE id = $2', [
      confidence,
      existing[0].id,
    ])
    return null
  ...
  await query(
    ...
    [
      ...
      pattern,
      toolSeqStr,
      JSON.stringify(conversation.slice(-5)),
      confidence,
    ],
  )
 ...
 * 列出待审批的 Skill
 */
export async function listPendingSkills(): Promise<
  Array<{
    id: string
    skill_name: string
    markdown_content: string
...
> {
  ...
  return rows.map((r) => ({
    ...r,
    trigger_words:
      typeof r.trigger_words === 'string' ? JSON.parse(r.trigger_words) : r.trigger_words || [],
  }))
}
 ...
 * 审批通过 - 激活 Skill
 */
export async function approveSkill(name: string, _approver: string): Promise<void> {
 ...
 * 审批拒绝
 */
export async function rejectSkill(name: string): Promise<void> {
  await query('UPDATE pending_skills SET status = $1 WHERE skill_name = $2 AND status = $3', [
    'rejected',
    name,
    'pending',
  ])
...
export async function evolveSkillFromConversation(
  ...
  console.log(
    `[SkillEvolution] Detected pattern with confidence ${detected.confidence}, pending approval: ${skillName}`,
  )
}
