...
export async function detectPatterns(
  ...
  if (existing.length > 0) {
    ...
    await query('UPDATE skill_patterns SET confidence = $1 WHERE id = $2', [confidence, existing[0].id]);
    return null;
  }
...
export async function approveSkill(name: string, _approver: string): Promise<void> {
  ...
  const triggerWords = typeof pending.trigger_words === 'string' ? JSON.parse(pending.trigger_words) : pending.trigger_words;
...
export async function rejectSkill(name: string): Promise<void> {
  await query('UPDATE pending_skills SET status = $1 WHERE skill_name = $2 AND status = $3', ['rejected', name, 'pending']);
}
