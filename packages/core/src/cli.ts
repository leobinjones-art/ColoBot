...
const HELP_TEXT = `
ColoMind - Multi-modal AI Assistant
...
const i18n = {
  ...
  en: {
    welcome: '\nWelcome to ColoMind! First run requires configuration.\n',
    ...
    configSaved: '\nConfiguration saved! Run colomind to start.\n',
    providerOptions: [
      { name: 'OpenAI', models: ['gpt-4o', 'gpt-4o-mini', 'gpt-4-turbo', 'gpt-3.5-turbo'] },
      {
        name: 'Anthropic',
...
async function startCli(): Promise<void> {
  ...
  if (!apiKey) {
    console.error('Error: No API key provided. Run `colomind init` first.')
    process.exit(1)
  }
  ...
  console.log('╔══════════════════════════════════════╗')
  console.log('║          ColoMind CLI Ready           ║')
  console.log('╚══════════════════════════════════════╝')
...
async function main() {
  ...
  // 读取配置中的语言设置
  const configPath = path.join(process.env.HOME || '', '.colomind', 'config.json')
