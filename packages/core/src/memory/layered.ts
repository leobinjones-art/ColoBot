...
export async function initLayeredMemoryTables(): Promise<void> {
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_working_session ON memory_working(agent_id, session_id)`);
}
...
export async function searchLongTermMemory(
  ...
  // 向量搜索
  const rows = await query<{ id: string; content: string; importance: number; metadata: Record<string, unknown>; created_at: Date; similarity: number }>(
...
export async function addEpisodicMemory(
  ...
  await query(
    `INSERT INTO memory_episodic (id, agent_id, content, embedding, decay_factor, metadata)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [id, agentId, content, embedding ? JSON.stringify(embedding) : null, 1.0, JSON.stringify(metadata)]
  );
...
export async function searchEpisodicMemory(
  ...
  const rows = await query<{ id: string; content: string; decay_factor: number; metadata: Record<string, unknown>; created_at: Date; similarity: number }>(
...
export async function clearWorkingMemory(
  ...
  await query(
    `DELETE FROM memory_working WHERE agent_id = $1 AND session_id = $2`,
    [agentId, sessionId]
  );
}
...
export async function getMemoryContext(
  ...
  // 相关记忆
  if (queryText) {
    const relevant = await searchAllMemory(agentId, queryText, { topK: 3, layers: ['long_term', 'episodic'] });
