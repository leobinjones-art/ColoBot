...
async function runTests() {
  ...
  const lines = testContent.split('\n').filter(line =>
    ...
    !line.startsWith('💡') &&
    !line.startsWith('🔍')
  ).slice(0, 200)  // 只取前 200 条

  console.log('══════════════════════════════════════════════════════════════')
  console.log(`快速三层防御测试 - 前 200 条`)
  ...
  for (const msg of lines) {
    ...
    try {
      ...
      if (stats.total % 20 === 0) {
        ...
        console.log(`进度: ${stats.total}/${lines.length} (${elapsed}s) | L1: ${stats.layer1Blocked} | L1.5: ${stats.layer15Blocked} | L2: ${stats.layer2Blocked} | 放行: ${stats.passed}`)
      }
    } catch (error) {
      stats.errors++
  ...
  console.log('\n放行案例分析（前10条）:')
  passedCases.slice(0, 10).forEach(r => {
    console.log(`  ✅ "${r.msg.slice(0, 60)}..."`)
  })
  if (passedCases.length > 10) {
    console.log(`  ... 还有 ${passedCases.length - 10} 条放行`)
  }
}
