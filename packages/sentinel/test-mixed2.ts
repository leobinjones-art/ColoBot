/**
 * 混合三层防御测试 - Layer 1.5 -> Layer 1 -> Layer 2
 * 先本地分析，再规则引擎，最后LLM推理
 */
...
async function runTests() {
  ...
  console.log('══════════════════════════════════════════════════════════════')
  console.log(`混合三层防御测试 - Layer 1.5 -> Layer 1 -> Layer 2`)
  ...
  console.log(`Layer 1.5 拦截 (最先): ${stats.layer15Blocked} (${(stats.layer15Blocked / stats.total * 100).toFixed(2)}%)`)
  console.log(`Layer 1 拦截 (第二): ${stats.layer1Blocked} (${(stats.layer1Blocked / stats.total * 100).toFixed(2)}%)`)
  console.log(`Layer 2 拦截 (最后): ${stats.layer2Blocked} (${(stats.layer2Blocked / stats.total * 100).toFixed(2)}%)`)
  console.log(`放行: ${stats.passed} (${(stats.passed / stats.total * 100).toFixed(2)}%)`)
  console.log(`错误: ${stats.errors}`)
  console.log('────────────────────────────────────────────────────────────')
  console.log(`总拦截率: ${((stats.layer15Blocked + stats.layer1Blocked + stats.layer2Blocked) / stats.total * 100).toFixed(2)}%`)
  ...
  // 保存结果
  fs.writeFileSync('./test-mixed2-results.json', JSON.stringify(results, null, 2))
  console.log('\n详细结果已保存到 test-mixed2-results.json')
}
