/**
 * 混合三层防御测试 - Layer 1.5 -> Layer 2 -> Layer 1
 * 先本地分析，再LLM推理，最后规则引擎
 */
...
async function runTests() {
  ...
  console.log('══════════════════════════════════════════════════════════════')
  console.log(`混合三层防御测试 - Layer 1.5 -> Layer 2 -> Layer 1`)
  ...
  console.log(`Layer 2 拦截 (第二): ${stats.layer2Blocked} (${(stats.layer2Blocked / stats.total * 100).toFixed(2)}%)`)
  ...
  console.log('────────────────────────────────────────────────────────────')
  console.log(`总拦截率: ${((stats.layer15Blocked + stats.layer2Blocked + stats.layer1Blocked) / stats.total * 100).toFixed(2)}%`)
  ...
  // 保存结果
  fs.writeFileSync('./test-mixed-results.json', JSON.stringify(results, null, 2))
  console.log('\n详细结果已保存到 test-mixed-results.json')
}
