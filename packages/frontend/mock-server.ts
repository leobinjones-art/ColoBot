...
const mockNotes = [
  { id: '1', title: '开发笔记', content: 'Vue 3 组合式 API 使用记录', tags: ['dev', 'vue'], createdAt: '2026-05-01', updatedAt: '2026-05-01' },
]
...
const mockFinances = [
  { id: '1', type: 'expense', amount: 35.5, category: '餐饮', note: '午餐', loggedAt: '2026-05-02T12:00:00' },
  { id: '2', type: 'income', amount: 10000, category: '工资', note: '4月工资', loggedAt: '2026-05-01T10:00:00' },
]
...
app.post('/api/v1/agents', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body } }))
...
app.post('/api/v1/todos', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body, status: 'pending' } }))
...
app.post('/api/v1/reminders', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body, status: 'pending' } }))
...
app.post('/api/v1/events', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body } }))
...
app.post('/api/v1/notes', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body, createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() } }))
...
app.post('/api/v1/contacts', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body } }))
...
app.post('/api/v1/habits', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body, streak: 0 } }))
...
app.post('/api/v1/moods', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body, loggedAt: new Date().toISOString() } }))
app.get('/api/v1/moods/stats', (req, res) => res.json({ code: 200, data: { average: 7, count: 10 } }))

// Finance API
app.get('/api/v1/finances', (req, res) => res.json({ code: 200, data: mockFinances }))
app.post('/api/v1/finances', (req, res) => res.json({ code: 200, data: { id: Date.now().toString(), ...req.body, loggedAt: new Date().toISOString() } }))
app.get('/api/v1/finances/stats', (req, res) => res.json({ code: 200, data: { totalIncome: 10000, totalExpense: 35.5 } }))
app.get('/api/v1/finances/monthly', (req, res) => res.json({ code: 200, data: [] }))

// Config API
app.get('/api/v1/config', (req, res) => res.json({ code: 200, data: { storage: {}, logging: { level: 'info' }, features: {} } }))
app.put('/api/v1/config', (req, res) => res.json({ code: 200, success: true }))
app.get('/api/v1/config/status', (req, res) => res.json({ code: 200, data: { server: 'running', database: 'connected', vectorStore: 'ready', version: '0.2.1' } }))
...
app.get('/api/v1/config/export', (req, res) => res.json({ code: 200, data: { config: {}, exportedAt: new Date().toISOString() } }))
app.get('/api/v1/config/models', (req, res) => res.json({
  code: 200,
  data: [
...
// User Profile API
app.get('/api/v1/user/profile', (req, res) => res.json({
  code: 200,
  data: {
    userId: 'user',
...
// Sentinel API
app.get('/api/v1/sentinel/status', (req, res) => res.json({
  code: 200,
  data: {
    status: 'healthy',
...
app.get('/api/v1/sentinel/sessions', (req, res) => res.json({
  code: 200,
  data: [
    { sessionId: 'sess-001', agentId: 'agent-1', status: 'active', lastHeartbeat: new Date().toISOString(), currentPhase: 'streaming' },
  ]
}))
...
app.post('/api/v1/chat/stream', (req, res) => {
  ...
  setTimeout(() => {
    ...
    if (message.includes('搜索') || message.includes('查询')) {
      res.write(`data: ${JSON.stringify({ type: 'tool_call_started', tool_name: 'web_search', arguments: JSON.stringify({ query: message }) })}\n\n`)

      setTimeout(() => {
        res.write(`data: ${JSON.stringify({ type: 'tool_call_completed', result: '找到 3 个相关结果' })}\n\n`)
