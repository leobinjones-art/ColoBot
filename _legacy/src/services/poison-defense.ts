 ...
 * AI 投毒检测
 */
export async function detectPoisoning(
  content: string,
): Promise<{ isPoison: boolean; reason?: string }> {
  // 快速规则检测
...
export async function getAgentTrustStatus(agentId: string): Promise<AgentTrustRecord | null> {
  return queryOne<AgentTrustRecord>('SELECT * FROM agent_trust_records WHERE agent_id = $1', [
    agentId,
  ])
}
  ...
  try {
    switch (contentType) {
      ...
      case 'knowledge':
        ...
        await query('DELETE FROM knowledge_base WHERE category = $1 AND name = $2', [
          category,
          name,
        ])
        break
