...
const feishuNotificationsAdapter: NotificationAdapter = {
  ...
  async send(payload: NotificationPayload) {
    ...
    const receiveId =
      (await getSetting(SETTINGS_KEYS.FEISHU_APPROVER_OPEN_ID)) ||
    ...
    const baseUrl =
      (await getSetting(SETTINGS_KEYS.COLOBOT_PUBLIC_URL)) ||
    ...
    console.log(
      `[FeishuNotification] Sent card for approval ${payload.approvalId}, message_id: ${messageId}`,
    )

    // 如果是 pending 状态，保存 message_id 以便后续更新
    if (payload.status === 'pending') {
      const { query } = await import('../memory/db.js')
      await query('UPDATE approval_requests SET feishu_message_id = $1 WHERE id = $2', [
        messageId,
        payload.approvalId,
      ])
    }
...
export function buildApprovalCard(
  ...
  baseUrl: string = 'http://localhost:18792',
): object {
