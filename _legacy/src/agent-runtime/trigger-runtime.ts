...
async function scheduleNext(trigger: Trigger): Promise<void> {
  ...
  // 持久化到 DB（供重启后补偿判断使用）
  await query('UPDATE triggers SET last_fired_at = NOW(), next_fire_at = $1 WHERE id = $2', [
    next,
    trigger.id,
  ])
...
function startIntervalTrigger(trigger: Trigger): void {
  ...
  if (trigger.next_fire_at) {
    ...
    if (nextMs <= now) {
      // 错过了，立即触发
      console.log(
        `[TriggerEngine] Interval trigger ${trigger.id} missed by ${now - nextMs}ms, firing now`,
      )
...
async function startCronTrigger(trigger: Trigger): Promise<void> {
  ...
  if (trigger.next_fire_at) {
    ...
    if (nextMs <= now) {
      // 错过了，立即触发一次
      console.log(
        `[TriggerEngine] Cron trigger ${trigger.id} missed by ${now - nextMs}ms, firing now`,
      )
      triggerTimers.set(trigger.id, { timeout: null, isRunning: true })
...
function matchesCron(now: Date, parts: string[]): boolean {
  ...
  const matchField = (
    field: string,
    value: number,
    extra?: { lastDay?: number; lastWeekday?: number },
  ): boolean => {
  ...
  return (
    ...
    matchField(hour, h) &&
    (dom === '*' ||
      matchField(dom, d, { lastDay: lastDayOfMonth, lastWeekday: getLastWeekdayOfMonth() })) &&
...
export async function fireTrigger(trigger: Trigger): Promise<void> {
  ...
  try {
    ...
    await executeSkill(
      {
        ...
        trigger_words:
          typeof skill.trigger_words === 'string'
            ? JSON.parse(skill.trigger_words)
            : skill.trigger_words || [],
        trigger_config:
          typeof skill.trigger_config === 'string'
    ...
    // 记录历史
    await query('INSERT INTO trigger_history (id, trigger_id, result) VALUES ($1, $2, $3)', [
      crypto.randomUUID(),
      trigger.id,
...
export async function fireWebhook(
  ...
  const trigger = await queryOne<TriggerRow>('SELECT * FROM triggers WHERE id = $1 AND type = $2', [
    triggerId,
    'webhook',
  ])
...
function evaluateCondition(
  ...
  switch (operator) {
    ...
    case 'now':
      ...
      const nowTimeConfig =
        (condition as Record<string, { after?: string; before?: string }>).now || {}
      const now = new Date()
