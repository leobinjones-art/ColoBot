...
async function findMatchingRule(toolName: string, argsStr: string): Promise<ApprovalRule | null> {
  ...
  for (const rule of rules) {
    try {
      ...
      if (
        rule.pattern_type === 'keyword' &&
        matchStr.toLowerCase().includes(rule.pattern.toLowerCase())
      )
        return rule
    } catch {}
  }
...
export async function getEvolutionStatus(ruleId: string): Promise<{
  ...
  const row = await queryOne<{
    user_approve_count: string
    user_reject_count: string
    auto_approve_threshold: string
    auto_reject_threshold: string
    confidence_decay_days: string
    last_decided_at: Date | null
  }>(
  ...
  try {
    ...
    const text =
      typeof response.content === 'string'
        ? response.content
        : response.content
            .map((b: ContentBlock) => (b.type === 'text' ? (b as TextContent).text : ''))
            .join('')
...
export async function checkDangerousLevel(call: ToolCall): Promise<DecisionResult> {
  ...
  if (tirithRule) {
    ...
    if (tirithRule.action === 'approve') {
      ...
      if (evo?.autoApprove) {
        console.log(
          `[ApprovalRules] Evolution auto_approve: ${tirithRule.name} (${evo.approveCount}x)`,
        )
        return { level: 'auto_approve', isCommercialDocument: false }
      }
      if (evo?.autoReject) {
        console.log(
          `[ApprovalRules] Evolution auto_reject: ${tirithRule.name} (${evo.rejectCount}x)`,
        )
  ...
  if (matchingRule) {
    ...
    if (evo) {
      if (evo.autoApprove) {
        console.log(
          `[ApprovalRules] Evolution auto_approve: ${matchingRule.name} (${evo.approveCount} approvals)`,
        )
        return { level: 'auto_approve', isCommercialDocument: false }
      }
      if (evo.autoReject) {
        console.log(
          `[ApprovalRules] Evolution auto_reject: ${matchingRule.name} (${evo.rejectCount} rejections)`,
        )
        return { level: 'auto_reject', isCommercialDocument: false }
      }
    }
 ...
 */
export async function recordToolHit(
  toolName: string,
  argsText: string,
  ruleId?: string,
): Promise<void> {
...
export async function isDangerousTool(toolName: string): Promise<boolean> {
  ...
  for (const rule of rules) {
    if (
      rule.pattern_type === 'keyword' &&
      toolName.toLowerCase().includes(rule.pattern.toLowerCase())
    ) {
      return true
    }
    if (rule.pattern_type === 'regex') {
...
const DEFAULT_RULES: SeedRule[] = [
  ...
  // ── P3: 商业文书（最低优先级）──────────────────────────────────────────
  {
    ...
    pattern:
      '合同|协议|contract|agreement|nda|保密协议|条款|terms|授权书|委托书|采购合同|销售合同|租赁合同|MoU',
    patternType: 'keyword',
    action: 'approve',
    riskLevel: 'high',
    priority: 50,
  ...
  for (const r of DEFAULT_RULES) {
    ...
    await query(
      ...
      [
        ...
        r.action,
        r.riskLevel,
        r.autoApproveThreshold ?? 3,
        r.autoRejectThreshold ?? 3,
        r.priority,
      ],
    )
  }
