...
export function registerTools(): void {
  ...
  registerTool('generate_video', async (args) => {
    ...
    const { prompt, model, first_frame_image, last_frame_image, subject_image } = args as {
      prompt: string
      model?: string
    ...
    if (createData.base_resp && createData.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax video task creation failed: ${createData.base_resp.status_code} ${createData.base_resp.status_msg}`,
      )
    }
    ...
    while (Date.now() < deadline) {
      ...
      if (statusData.status === 'fail' || statusData.status === 'failed') {
        throw new Error(
          `MiniMax video generation failed: ${statusData.base_resp?.status_msg || statusData.status}`,
        )
      }
    }
  ...
  registerTool('query_video_task', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax video query failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
