...
export async function getUserProfile(agentId: string): Promise<UserProfile | null> {
  ...
  return {
    ...
    communication_style:
      (row.communication_style as 'formal' | 'casual' | 'technical') || undefined,
...
function buildProfileSummary(profile: UserProfile): string {
  ...
  if (profile.current_projects?.length)
    parts.push(`当前项目：${profile.current_projects.join('、')}`)
...
export function buildProfilePrompt(profile: UserProfile | null): string {
  ...
  if (profile.role) {
    parts.push(
      `用户是一位${getRoleLabel(profile.role)}${profile.organization ? `，来自${profile.organization}` : ''}。`,
    )
  }
...
export async function evolveProfileFromConversation(
  ...
  if (lastAnalyzedAt) {
    const hoursSinceLastUpdate =
      (Date.now() - new Date(lastAnalyzedAt).getTime()) / (1000 * 60 * 60)
