  ...
  if (
    ...
    try {
      if (fs.existsSync(fullPath)) {
        ...
        res.writeHead(200, {
          'Content-Type': contentTypes[ext || 'html'] || 'text/plain; charset=utf-8',
        })
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (path === '/api/agents' && method === 'POST') {
      ...
      const agentInput = {
        ...
        system_prompt_override: body.system_prompt_override
          ? String(body.system_prompt_override)
          : undefined,
      }
    ...
    if (path === '/api/chat' && method === 'POST') {
      ...
      if (triggeredSkill) {
        const response = await executeSkill(triggeredSkill, agent_id, {
        ...
        const result = await runAgent({
          agentId: agent_id,
          sessionKey: session_key,
          userMessage: message as string | ContentBlock[],
          ipAddress: clientIp,
        })
    ...
    if (skillDetailMatch && method === 'PUT') {
      ...
      await query(
        ...
        [
          ...
          description,
          markdown_content,
          trigger_words ? JSON.stringify(trigger_words) : null,
          enabled,
          skillId,
        ],
      )
    ...
    if (path === '/api/skills/import/clawhub' && method === 'POST') {
      const body = await parseBody(req)
      const { importFromClawHub, importFromClawHubUrl } =
    ...
    if (path.match(/^\/api\/approvals\/([^/]+)\/(approve|reject)$/)) {
      ...
      if (action === 'approve') {
        ...
        result = await approvalFlow.approve(
          id,
          approver,
    ...
    if (path.match(/^\/api\/security\/poisoning-attempts\/[^/]+$/) && method === 'GET') {
      ...
      const row = await queryOne<{ id: string; source: string; issues: string }>(
        'SELECT * FROM poisoning_attempts WHERE id = $1',
        [id],
      )
      if (!row) {
        res.writeHead(404, { 'Content-Type': 'application/json' })
    ...
    if (path === '/api/security/write-audit' && method === 'GET') {
      ...
      const rows = await query<{ id: string; source: string }>(
        'SELECT * FROM content_write_audit ORDER BY created_at DESC LIMIT $1',
        [limit],
      )
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(
        JSON.stringify(
          rows.map((r: { id: string; source: string }) => ({
            ...r,
    ...
    if (path === '/api/settings/subagent' && method === 'PUT') {
      ...
      if (body.allowedTools !== undefined) {
        await setSetting(
          SETTINGS_KEYS.SUBAGENT_ALLOWED_TOOLS,
          JSON.stringify(body.allowedTools),
          'SubAgent allowed tools whitelist',
        )
      }
      if (body.blockedTools !== undefined) {
        await setSetting(
          SETTINGS_KEYS.SUBAGENT_BLOCKED_TOOLS,
          JSON.stringify(body.blockedTools),
          'SubAgent blocked tools blacklist',
        )
      }
      if (body.defaultTtlMs !== undefined) {
        await setSetting(
          SETTINGS_KEYS.SUBAGENT_DEFAULT_TTL_MS,
          String(body.defaultTtlMs),
          'SubAgent default TTL in ms',
        )
      }
      res.writeHead(200, { 'Content-Type': 'application/json' })
      res.end(JSON.stringify({ ok: true }))
    ...
    if (path.startsWith('/api/workspace/') && method === 'GET') {
      ...
      if (parts.length >= 4 && parts[2] === 'workspace') {
        ...
        const { getSubAgent, getSubAgentWorkspacePath } =
          await import('./agent-runtime/sub-agents.js')
        ...
        import('fs/promises').then(async (fsP) => {
          try {
            ...
            const filePath = isDownload && parts[5] ? '/' + parts.slice(5).join('/') : '/'
    ...
    if (path.startsWith('/api/workspace/') && method === 'POST') {
      ...
      if (parts.length >= 5 && parts[2] === 'workspace' && parts[4] === 'files') {
        ...
        // 获取上传的文件
        const { parseMultipart } = await import('./utils/multipart.js').catch(() => ({
          parseMultipart: null,
        }))
        if (!parseMultipart) {
          res.writeHead(500, { 'Content-Type': 'application/json' })
    ...
    if (path === '/api/llm/test' && method === 'POST') {
      ...
      try {
        if (provider === 'openai') {
          ...
          const endpoint =
            process.env.MINIMAX_STREAM_ENDPOINT ||
    ...
    if (path.startsWith('/api/settings/sop/subagents/') && method === 'PUT') {
      ...
      const body = await parseBody(req)
      await saveSubAgentConfigToDb(
        type,
        (
          body as {
            config: { personality: string; rules: string[]; skills: string[]; tools: string[] }
          }
        ).config,
      )
      res.writeHead(200, { 'Content-Type': 'application/json' })
  ...
  ws.on('message', async (data) => {
    try {
      ...
      if (msg.type === 'chat') {
        ...
        if (triggeredSkill) {
          // Skill 执行（非流式）
          const response = await executeSkill(triggeredSkill, agentId, {
            sessionKey,
