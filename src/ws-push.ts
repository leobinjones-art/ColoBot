...
export function pushWsResult(agentId: string, sessionKey: string, result: unknown): void {
  if (!wsClients) return
...
export function pushWsChunk(agentId: string, sessionKey: string, chunk: string): void {
  if (!wsClients) return
  const clientId = `${agentId}:${sessionKey}`
  const ws = wsClients.get(clientId)
  try {
...
export function pushWsDone(agentId: string, sessionKey: string): void {
  if (!wsClients) return
  const clientId = `${agentId}:${sessionKey}`
  const ws = wsClients.get(clientId)
  try {
