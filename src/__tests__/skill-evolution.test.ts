...
describe('Skill Evolution', () => {
  ...
  describe('detectPatterns', () => {
    ...
    it('should increase confidence for existing pattern', async () => {
      ...
      const result = await detectPatterns('agent-1', 'Agent', [], ['tool1', 'tool2'])
  ...
  describe('generateSkillMarkdown', () => {
    it('should generate skill markdown', () => {
      const markdown = generateSkillMarkdown('TestSkill', '工具序列: tool1 → tool2', [
        'tool1',
        'tool2',
      ])
    ...
    it('should include usage scenario', () => {
      const markdown = generateSkillMarkdown('MySkill', 'Pattern description', ['toolA'])
  ...
  describe('writePendingSkill', () => {
    it('should write pending skill', async () => {
      const id = await writePendingSkill('agent-1', 'TestSkill', '# TestSkill\nContent', ['tool1'])
  ...
  describe('rejectSkill', () => {
    it('should update skill status to rejected', async () => {
      ...
      expect(query).toHaveBeenCalledWith(expect.stringContaining('UPDATE pending_skills'), [
        'rejected',
        'TestSkill',
        'pending',
      ])
