...
export class ApprovalFlow {
  ...
  private async notifyExternal(
    payload: Parameters<typeof import('../services/notifications.js').sendApprovalNotification>[0],
  ): Promise<void> {
    try {
  ...
  async get(id: string): Promise<ApprovalRequest | null> {
    const row = await queryOne<ApprovalRequest>('SELECT * FROM approval_requests WHERE id = $1', [
      id,
    ])
  ...
  async executeApproved(
    id: string,
  ): Promise<{ approval: ApprovalRequest | null; toolResult?: unknown; error?: string }> {
  ...
  private async notifyWebSocket(
    ...
    try {
      ...
      if (rows.length === 0) {
        ...
        console.warn(
          `[Approval] No pending_conversation for approval ${approvalId}, skipping WebSocket push`,
        )
        return
      }
