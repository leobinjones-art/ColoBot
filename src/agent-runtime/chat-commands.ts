 ...
 * /compact - 压缩上下文
 */
async function handleCompact(context: {
  agentId: string
  sessionKey: string
}): Promise<CommandResult> {
  ...
  await sessionManager.updateContext(context.agentId, context.sessionKey, { history: newHistory })
...
function handleReasoning(args: string | undefined): CommandResult {
  ...
  return {
    success: true,
    message: enabled ? '✅ 推理展示已开启（需要模型支持）' : '✅ 推理展示已关闭',
    action: 'reasoning',
 ...
 * /approvals 或 /pending - 查看待审批列表
 */
async function handlePendingApprovals(context: { agentId: string }): Promise<CommandResult> {
  try {
    const { approvalFlow } = await import('./approval.js')
