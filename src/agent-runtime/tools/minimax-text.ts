...
export function registerTools(): void {
  ...
  registerTool('generate_image', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax image generation failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
  ...
  registerTool('vision', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax vision failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
