  ...
  registerTool('send_message', async (args) => {
    ...
    switch (channel) {
      ...
      case 'feishu': {
        ...
        if (!webhook)
          throw new Error('webhook_url is required or FEISHU_WEBHOOK_URL not configured')
      ...
      case 'telegram': {
        ...
        if (!tg.botToken || !chatId) {
          throw new Error(
            'Telegram not configured. Set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID in Dashboard.',
          )
        }
