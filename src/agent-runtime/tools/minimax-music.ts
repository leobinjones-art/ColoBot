...
export function registerTools(): void {
  ...
  registerTool('generate_music', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax music failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
  ...
  registerTool('generate_music_cover', async (args) => {
    ...
    const { prompt, audio_url, lyrics, model, seed, output_format } = args as {
      prompt: string
      audio_url: string
      lyrics?: string
      model?: string
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax music cover failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
