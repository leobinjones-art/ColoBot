...
export function registerTools(): void {
  ...
  registerTool('upload_file', async (args) => {
    ...
    if (!purpose)
      throw new Error(
        'purpose is required (voice_clone/prompt_audio/t2a_async_input/video_generation)',
      )
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax file upload failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
  ...
  registerTool('list_files', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax file list failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
  ...
  registerTool('retrieve_file', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax file retrieve failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
  ...
  registerTool('minimax_delete_file', async (args) => {
    ...
    if (data.base_resp && data.base_resp.status_code !== 0) {
      throw new Error(
        `MiniMax file delete failed: ${data.base_resp.status_code} ${data.base_resp.status_msg}`,
      )
    }
