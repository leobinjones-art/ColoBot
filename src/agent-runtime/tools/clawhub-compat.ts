...
export function toClawHubSkill(skill: ColoBotSkill): string {
  ...
  const markdown =
    skill.markdown_content || `# ${skill.name}\n\n${skill.description || 'A ColoBot skill.'}`
 ...
 */
export async function importFromClawHub(content: string, agentId?: string): Promise<ColoBotSkill> {
  ...
  await query(
    ...
    [
      ...
      colobot.name,
      colobot.description || '',
      colobot.markdown_content,
      JSON.stringify(colobot.trigger_words),
      JSON.stringify(colobot.trigger_config),
    ],
  )
  ...
  if (colobot.clawhub_slug) {
    await query(
      ...
      [
        crypto.randomUUID(),
        `clawhub_import:${colobot.clawhub_slug}`,
        JSON.stringify({ slug: colobot.clawhub_slug, version: colobot.version }),
        [],
        [],
      ],
    )
...
export async function exportToClawHub(skillId: string): Promise<string> {
  ...
  const skill: ColoBotSkill = {
    ...
    trigger_words:
      typeof row.trigger_words === 'string'
        ? JSON.parse(row.trigger_words)
        : row.trigger_words || [],
    enabled: true,
  }
