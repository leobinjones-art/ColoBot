...
export async function handleSopFlow(
  ...
  if (!state) {
    ...
    console.log(
      '[SOP Handler] Analysis result:',
      analysis.isAcademicTask,
      analysis.taskType,
      analysis.researchPurpose,
    )

    // 判断是否触发 SOP：学术任务 或 有明确研究目的（paper/research/learning）
    const shouldTriggerSop =
      analysis.isAcademicTask ||
      (analysis.researchPurpose &&
        ['paper', 'research', 'learning'].includes(analysis.researchPurpose))
  ...
  if (state && state.taskId.startsWith('temp-')) {
    ...
    if (purpose) {
      ...
      // 重新让AI根据目的生成步骤
      const purposeText =
        purpose === 'paper' ? '写论文' : purpose === 'research' ? '做研究' : '学习'
