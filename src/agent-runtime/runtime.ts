 ...
 * - 审批/待审批 - 列出待审批列表
 */
function detectInChatApproval(
  messageText: string,
): { action: 'approve' | 'reject' | 'list'; approvalId?: string } | null {
  // 列出待审批
...
export async function runAgent(opts: RunOptions): Promise<RunResult | PendingResult> {
  ...
  if (threat.isThreat) {
    ...
    await writeAudit({
      ...
      detail: {
        type: threat.type,
        pattern: threat.matchedPattern,
        messageLength: messageTextStr.length,
      },
      ipAddress,
      result: 'blocked',
    })
  ...
  for (let round = 0; round < maxRounds; round++) {
    ...
    const allowedCalls = toolCalls.filter(
      (c) => isToolAllowed('__parent__', c.name) && !DANGEROUS_TOOLS[c.name],
    )
...
export async function runAgentStream(opts: RunStreamOptions): Promise<void> {
  const {
    agentId,
    sessionKey,
    userMessage,
...
export async function continueRun(
  ...
  }>('SELECT * FROM pending_conversations WHERE approval_id = $1', [approvalId])
