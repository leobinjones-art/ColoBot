...
export const zhMessages: I18nMessages = {
  sop: {
    ...
    paused: (currentStep, totalSteps) =>
      `⏸️ SOP 流程已暂停。进度：步骤 ${currentStep}/${totalSteps}。发送"继续"恢复。`,
    resumed: (currentStep, totalSteps) =>
      `🔄 SOP 流程已恢复。当前步骤：${currentStep}/${totalSteps}`,
    restarted: (step) => `🔄 步骤 ${step} 已重启。`,
    completed: '🎉 所有步骤已完成！是否生成最终输出（论文/报告）？回复"是"或"否"。',
    purposeSelection: (taskName) =>
      `检测到学术任务：**${taskName}**\n\n请选择研究目的：\n\n1. **写论文** - 发表期刊/毕业论文\n2. **做研究** - 科学研究、实验、分析\n3. **学习** - 学习某个领域的知识\n\n请回复数字或描述您的目的。`,
    ...
    finalOutputGenerated: (downloadUrl) =>
      downloadUrl ? `✅ 报告已生成！[下载](${downloadUrl})` : '✅ 报告已生成！',
    taskList: (tasks) =>
      tasks.length > 0
...
export const enMessages: I18nMessages = {
  sop: {
    ...
    paused: (currentStep, totalSteps) =>
      `⏸️ SOP workflow paused. Progress: Step ${currentStep}/${totalSteps}. Send "continue" to resume.`,
    resumed: (currentStep, totalSteps) =>
      `🔄 SOP workflow resumed. Current step: ${currentStep}/${totalSteps}`,
    ...
    finalOutputReady:
      '🎉 All steps completed! Generate final output (paper/report)? Reply "yes" or "no".',
    finalOutputGenerated: (downloadUrl) =>
      downloadUrl ? `✅ Report generated! [Download](${downloadUrl})` : '✅ Report generated!',
    taskList: (tasks) =>
      tasks.length > 0
        ? `📋 **Active Tasks**\n\n${tasks.map((t, i) => `${i + 1}. ${t.taskName} (${t.status})`).join('\n')}`
  ...
  errors: {
    ...
    subAgentLimitReached: (limit) =>
      `Sub-agent concurrency limit reached (${limit}). Please try again later.`,
  },
  commands: {
    modelSwitch: (model) =>
      `Model switching requires updating Agent configuration.\nRequested: ${model}\n\nTip: Modify the Agent's primary_model_id in Dashboard.`,
